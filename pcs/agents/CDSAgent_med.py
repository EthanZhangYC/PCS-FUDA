...
from models import (CosineClassifier, MemoryBank, SSDALossModule,
                        ...
                        update_data_memory, SegMemoryBank)
...
from models.unet import UNet, OUNet
from utils.data_mmwhs import WHS_dataset
...
class CDSAgentMed(BaseAgent):
    def __init__(self, config):
        ...
        super(CDSAgentMed, self).__init__(config)
        ...
        # init loss
        loss_fn = SSDALossModule(self.config, gpu_devices=self.gpu_devices).cuda()
        ...
        self.dice_loss = DiceLoss(n_classes=5)
    ...
    def _init_labels(self):
        ...
        if self.fewshot:
            self.predict_ordered_labels_pseudo_source = (
                ...
                torch.zeros([train_len_src, self.img_size, self.img_size], dtype=torch.long).detach().cuda() - 1
            )
            for ind, lbl in zip(self.fewshot_index_source, self.fewshot_label_source):
                self.predict_ordered_labels_pseudo_source[ind] = torch.tensor(lbl)
        self.predict_ordered_labels_pseudo_target = (
            # torch.zeros(train_len_tgt, dtype=torch.long).detach().cuda() - 1
            torch.zeros([train_len_tgt, self.img_size, self.img_size], dtype=torch.long).detach().cuda() - 1
        )
    ...
    def _load_datasets(self):
        ...
        self.img_size = self.config.data_params.img_size
        ...
        self.data_path_train_unl={
            'source':self.config.data_params.data_path_train_source_unl,
            'target':self.config.data_params.data_path_train_target_unl
        }
        for domain_name in ("source", "target"):
            ...
            # )
            train_dataset = WHS_dataset(self.data_path_train_unl[domain_name], transforms=True)
        ...
        if fewshot:
            ...
            #     image_size=image_size,
            # )
            train_lbd_dataset_source = WHS_dataset(self.config.data_params.data_path_train_source_lbd, transforms=True, labeled=True)
            ...
            #     image_size=image_size,
            # )
            test_unl_dataset_source = WHS_dataset(self.config.data_params.data_path_val_source_unl, labeled=True)
            ...
            #     image_size=image_size,
            # )
            train_lbd_dataset_source = WHS_dataset(self.config.data_params.data_path_train_source_lbd, transforms=True)
        ...
        #     image_size=image_size,
        # )
        test_unl_dataset_target = WHS_dataset(self.config.data_params.data_path_val_target_unl, labeled=True)
    ...
    def _create_model(self):
        ...
        arch = self.config.model_params.arch
        self.num_class = 5
        ...
        if arch == 'UNet':
            ...
            model = UNet(in_chns=1, class_num=self.num_class).cuda()
        if arch == 'OUNet':
            model = OUNet(in_chns=1, class_num=self.num_class, out_dim=out_dim).cuda()
        elif arch == 'FCN':
            model = FCN(num_cls=self.num_class, pretrained=True, weights_init=None).cuda()
        elif arch == 'DRN':
            model = DRN(num_cls=self.num_class, weights_init=None).cuda()
        ...
        for batch_i in range(num_batches):
            ...
            for domain_name in ("source", "target"):
                ...
                if self.cls and domain_name == "source":
                    ...
                    images_lbd = images_lbd.float().cuda()
                    labels_lbd = labels_lbd.cuda()
                    out_lbd,feat_lbd = self.model(images_lbd)
                ...
                if (self.tgt and domain_name == "target") or self.ssl:
                    ...
                    out_unl,feat_unl = self.model(images_unl)
                ...
                if self.semi and domain_name == "source":
                    ...
                    indices_semi = indices_unl[semi_mask] # idx not in fewshot list
                ...
                for ind, ls in enumerate(loss_list):
                    ...
                    if ls == "cls-dice":
                        loss_part, _ = self.dice_loss(out_lbd, labels_lbd)
                    elif ls == "cls-so" and domain_name == "source":
    ...
    def _load_fewshot_to_cls_weight(self):
        ...
        if method in ["fewshot", "fewshot-tgt"]:
            ...
            mask = (self.predict_ordered_labels_pseudo_source != -1).view(-1, self.img_size*self.img_size).any(dim=-1)
        ...
        if is_tgt:
            ...
            mask = (self.predict_ordered_labels_pseudo_target != -1).view(-1,self.img_size*self.img_size).any(dim=-1)
    ...
    def score(self, loader, name="test"):
        ...
        dice_val = np.zeros(self.num_class)
        for batch_i, (indices, images, labels) in enumerate(loader):
            images = images.float().cuda()
            labels = labels.cuda()

            pred,feat = self.model(images)
            ...
            for i in range(self.num_class):
                ...
                label_batch_copy= labels.cpu().numpy().copy()
                label_batch_copy[label_batch_copy != i] = 0
                dice_val[i] += mmb.dc(outputs_copy, label_batch_copy)
            
        # self.summary_writer.add_scalars('info/val_dice', (dice_val[1:].mean())/len(loader), self.current_epoch)
        self.logger.info('epoch %d : val_dice: %f' % (self.current_epoch, (dice_val[1:].mean())/len(loader)))
        performance = (dice_val[1:].mean())/len(loader)
    ...
    def compute_train_features(self):
        ...
        for domain in ("source", "target"):
            ...
            # train_loader = self.train_lbd_loader_source
            features, y, idx = [], [], []
            tqdm_batch = tqdm(
    ...
    def _init_memory_bank(self):
        # self.img_size = 256
        out_dim = self.config.model_params.out_dim
        # bank_len = self.config.model_params.bank_len
        for domain_name in ("source", "target"):
            data_len = self.get_attr(domain_name, "train_len")
            # memory_bank = MemoryBank(data_len*self.img_size*self.img_size, out_dim)
            memory_bank = SegMemoryBank(data_len, out_dim, self.img_size)
            if self.config.model_params.load_memory_bank:
                ...
                # feat = feat.reshape(-1,out_dim)
                # pdb.set_trace()
                # print(idx.shape,feat.shape)
        ...
        for clus_type in self.config.loss_params.clus.type:
            ...
            if clus_type == "each":
                for domain_name in ("source", "target"):
                    ...
                    memory_bank_tensor_per_pixel = memory_bank_tensor.transpose(1,3).reshape(-1,self.config.model_params.out_dim)

                    # clustering
                    cluster_labels, cluster_centroids, cluster_phi = torch_kmeans(
                        k_list,
                        memory_bank_tensor_per_pixel,
